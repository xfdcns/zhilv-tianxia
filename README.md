...
| 方向 | 状态 | 说明 |
|:---|:---|:---|
| Redis 缓存层 | ✅ 已完成 | 天气/地图/RAG/Rerank 四层缓存 |
| LLM Query Rewrite | ✅ 已完成 | Top1 命中率 80%→86.7%，MRR 0.889→0.922 |
| Cross-encoder Rerank | ✅ 已完成 | Top1 命中率 86.7%→93.3%，MRR 0.922→0.967 |
| Token 观测 | ✅ 已完成 | 分项统计 + `/trip/stats` 汇总 |
| Token 成本分析看板 | 🚧 规划中 | 前端可视化，对比不同 RAG 策略下的消耗/延迟/质量 |
| 混合检索（向量 + BM25） | 🚧 规划中 | RRF 融合排序，覆盖语义+关键词双场景 |
| 检索结果压缩去冗 | 🚧 规划中 | 送入 LLM 前压缩去重，减少 token 消耗 |
| LangGraph 工作流 | 🚧 规划中 | 状态机编排 + LLM 意图路由 + 条件分支/并行 |
| GraphRAG | 🚧 规划中 | 城市/景点/路线/标签图结构，多地点联动推荐 |
| 联网搜索增强 | 🚧 规划中 | 补充实时营业状态、热门地点、节假日信息 |
